...
Идея простая:
- во ВСЕХ основных потоках по‑прежнему используется Cloud.ru LLM (эмбеддинги + основная генерация),
    ...
    async def get_completion(self, prompt: str, max_tokens: int = 1000) -> str:
        ...
        if not self.enabled or not self.url:
            return (
                ...
                "Продолжаю работать только через Cloud.ru LLM."
            )
        ...
        try:
            ...
            return (
                ...
                "Используйте ответ от Cloud.ru LLM."
            )

        try:
            choices = data.get("choices") or []
            if not choices:
                return "Локальная LLM вернула пустой ответ. Используйте ответ от Cloud.ru LLM."
